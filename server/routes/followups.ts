...
// POST /api/followups
followupsRouter.post('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const data = req.body;
...
followupsRouter.patch('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { status, lastVisitNotes } = req.body;
    const fup = await getDocById('followUps', req.params.id);
    if (!fup) {
      return res.status(404).json({ success: false, error: 'Follow-up task not found' });
    }
    ...
    if (status === 'completed') {
      ...
      await setDocument('healthRecords', evtId, {
        ...
        providerRole: 'ASHA Frontline Worker',
        eventType: 'follow_up_completed',
        title: 'Community Follow-up Completed by ASHA',
        notes: `ASHA Note: ${lastVisitNotes}. Home check in Village ${fup.patientVillage}.`,
        badgeType: 'success'
      });
      ...
      await setDocument('notifications', notifId, {
        ...
        message: `ASHA completed follow-up visit for ${fup.patientName} (${fup.patientVillage}).`,
        type: 'follow_up',
        read: false,
        timestamp: 'Just now'
      });
